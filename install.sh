#!/usr/bin/env bash
#
# Drishti one-shot installer for AWS.
#
...
#
#   curl -sL https://drishti.run/install.sh | bash -s -- --region ap-southeast-2
#
# What this installs:
...
#     more. Drishti reads configuration to assess it; it never writes to your account,
...
DEFAULT_IMAGE="public.ecr.aws/w4o8p5x9/drishti:latest"
TEMPLATE_URLS=(
  "https://drishti.run/drishti-aws.yaml"
  "https://raw.githubusercontent.com/abhijitsghosh/drishti-deploy/main/drishti-aws.yaml"
)
...
echo "▶ Installing Drishti into AWS account: $ACCOUNT (region $REGION)"
...
# client uses /login/oauth2/code/{registrationId}, and Drishti's registration id is "oidc".
...
✅ Drishti is installed — everything is in CloudFormation stack '$STACK'.
   ...
   Drishti runs as '$ROLE' with SecurityAudit + ViewOnlyAccess and nothing more. It reads
   ...
   Upgrade later (image only, keeps your attestations and decisions):
     curl -sL https://drishti.run/upgrade.sh | bash -s -- --region $REGION
